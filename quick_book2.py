...
# time.sleep(0.3)
'#week_main > tbody > tr:nth-child(26) > td.I.writable > div > div.celldiv.slots7.clone.ui-resizable > a'
talble_pos = ''
column = 0
for i in range(2, 9):
    talble_pos = '#week_main > tbody > tr:nth-child(' + str(book_time_row[your_book_time]) + ') > td:nth-child(' + str(i) + ')'

    try:
        element = wd.find_element_by_css_selector(talble_pos).click()
        ...
        try:
            element = wd.find_element_by_css_selector("#submit_buttons > form > fieldset > input[type=submit]").click()
        except:
            column = i
            break
    except:
...
'#submit_buttons > form > fieldset > input[type=submit]'
